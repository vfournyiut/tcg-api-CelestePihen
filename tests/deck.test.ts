...
describe('GET /decks/:id', () => {
  ...
  it('should return 404 for invalid deck id', async () => {
    const response = await request(app).get('/api/decks/invalid')
  ...
  it('should return 403 when deck does not belong to user', async () => {
    ...
    expect(response.body).toHaveProperty(
      'error',
      "Ce deck n'appartient pas à l'utilisateur",
    )
  })
...
describe('DELETE /decks/:id', () => {
  ...
  it('should return 404 for invalid deck id', async () => {
    const response = await request(app).delete('/api/decks/invalid')
